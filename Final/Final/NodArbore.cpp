...
void Afisare(NodArbore * nod, int nivel, int childNumber)
{
	printf("Level = %d child = %d %s\n", nivel, childNumber, toString(nod));
	for (int i = 0; i < 4; i++)
	{
		NodArbore* copil = nod->copii[i];
		if (copil != nullptr)
		{
			Afisare(copil, nivel + 1, i);
		}
		else
			return;
	}
...
void Afisare(NodArbore * nod, int nivel, int childNumber,HDC hdc, int startX,int startY,int arcLength)
{
	if (nod == NULL)
		return;
	//printf("Level = %d child = %d %s\n", nivel, childNumber, toString(nod));
	...
	char temp[10];
	wchar_t wtext[10];
	char* str_nod = toString(nod);
	mbstowcs(wtext, str_nod, strlen(temp) + 1);
	TextOut(hdc, startX-10, startY-6, wtext, strlen(str_nod) + 1);

	hLinePen = CreatePen(PS_SOLID, 2, qLineColor);
	hPenOld = (HPEN)SelectObject(hdc, hLinePen);
	Arc(hdc, startX - arcLength, startY - arcLength / 2, startX + arcLength, startY + arcLength / 2, 0, 0, 0, 0);
	SelectObject(hdc, hPenOld);
	DeleteObject(hLinePen);

	// print the node in the curent configuration
	for (int i = 0; i < 4; i++)
	{
		NodArbore* copil = nod->copii[i];
		if (copil != nullptr)
		{
			int newLevel = nivel + 1;
			int indexX = (newLevel+2) * 30;
			int index = -200;
			if (i == 1)
				index = -80;
			if (i == 2)
			...
			if (i == 3)
				index = 180;
			hPenOld = (HPEN)SelectObject(hdc, hLinePen);
			MoveToEx(hdc, startX+5, startY+5, NULL);
			LineTo(hdc, startX + index, startY + indexX);
			Afisare(copil, newLevel, i, hdc,startX+index, startY+indexX, arcLength);
		}
		else
			return;
	}
...
char* toString(NodArbore* nod)
{
	if (nod == NULL)
		return NULL;
	char* temp = (char*)my_malloc(100);
	char* temp_less = (char*)malloc(40);
	short index = 0;
	strcpy(temp, "");
	for (int i = 0; i < 3; i++)
	{
		if (nod->chei[i] != 0x00)
		{
			sprintf(temp_less, "%d,", nod->chei[i]->value);
			strcat(temp + index, temp_less);
			index += strlen(temp_less);
